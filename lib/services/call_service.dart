...
class CallService extends ChangeNotifier {
  ...
  Future<void> start({required String targetId, required String targetName, required bool withVideo, required String id}) async {
    if (active) return;
    peerId = targetId; peerName = targetName; video = withVideo; callId = id; incoming = false;
  ...
  Future<void> receive(String fromId, String fromName, Map<String, dynamic> signal) async {
    ...
    if (kind == 'offer') {
      ...
      peerId = fromId; peerName = fromName; callId = id; video = signal['video'] == true; incoming = true; _pendingOffer = signal; notifyListeners(); return;
    }
  ...
  Future<void> end({bool notifyPeer = true}) async {
    ...
    if (notifyPeer && target != null && id != null) await _sendSignal(target, {'kind': 'hangup', 'callId': id});
    for (final track in _localStream?.getTracks() ?? const <MediaStreamTrack>[]) { await track.stop(); }
    await _localStream?.dispose(); await _connection?.close();
    _localStream = null; _connection = null; localRenderer.srcObject = null; remoteRenderer.srcObject = null;
    peerId = null; peerName = null; callId = null; incoming = false; connected = false; _pendingOffer = null; _pendingCandidates.clear();
    notifyListeners();
  }
  ...
  Future<void> _prepareConnection() async {
    ...
    _connection!.onConnectionState = (state) { connected = state == RTCPeerConnectionState.RTCPeerConnectionStateConnected; notifyListeners(); };
  }
